...
const int  USE_PRIMITIVE_RESTART = 1; //开启图元中断
...
void init(void)
{
	...
	glGenVertexArrays(1, &VAO);  //创建VAO管理状态
	glBindVertexArray(VAO);

	glGenBuffers(1, &EBO);  //建立索引缓存
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices),
		          indices, GL_STATIC_DRAW);

	glGenBuffers(1, &VBO); //产生VBO，将顶点数据传入
...
void keyboard(unsigned char key, int x, int y)
{
	switch(key) {
	    case 'm': {  //m改变模型显示方式
		   ...
		   glPolygonMode(GL_FRONT_AND_BACK, mode);  //改变现实模式
       }
	    break;
	}
}
